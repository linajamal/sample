...
class hr_contract(models.Model):
    ...
    weekends_per_month = fields.Integer(string='No. of weekends per month',
                                          ...
                                          readonly=False)
    @api.depends('date_start','date_end','end_service')
    @api.one
    ...
    @api.onchange('end_service')
    def onchange_end_service(self):
        for rec in self:
            rec.eos_structure_id= self.env['res.company']._company_default_get().salary_structure_id.id
